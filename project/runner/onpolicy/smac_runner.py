...
import torch
from project.runner.onpolicy.base_runner import Runner
...
class SMACRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            if self.use_linear_lr_decay:  # 学习率线性衰减
    ...
    def collect(self, step):
        ...
        value, action, action_log_prob, rnn_state, rnn_state_critic = self.trainer.policy.get_actions(np.concatenate(self.buffer.share_obs[step]),
                                                                                                      np.concatenate(
        ...
        rnn_states = np.array(
            np.split(_t2n(rnn_state), self.n_rollout_threads))
    ...
    def insert(self, data):
        ...
        dones_env = np.all(dones, axis=1)  # (n_rollout_threads, n_agents)
        ...
        active_masks[dones_env == True] = np.zeros(
            ((dones_env == True).sum(), self.num_agents, 1), dtype=np.float32)

        bad_masks = np.array(
            [[[0.0] if info[agent_id]["bad_transition"] else [1.0] for agent_id in range(self.num_agents)] for info in infos])
