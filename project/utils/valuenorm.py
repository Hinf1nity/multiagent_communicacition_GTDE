...
class ValueNorm(nn.Module):
    ...
    def __init__(self, input_shape, norm_axes=1, beta=0.99999, per_element_update=False, epsilon=1e-5):
        ...
        self.running_mean = nn.Parameter(
            torch.zeros(input_shape), requires_grad=False).to(**self.tpdbv)
        self.running_mean_sq = nn.Parameter(
            torch.zeros(input_shape), requires_grad=False).to(**self.tpdbv)
        self.debiasing_term = nn.Parameter(
            torch.tensor(0.0), requires_grad=False).to(**self.tpdbv)
    ...
    def update(self, input_vector):
        ...
        input_vector = input_vector.to(**self.tpdbv)
    ...
    def denormalize(self, input_vector):
        ...
        input_vector = input_vector.to(**self.tpdbv)
        mean, var = self.running_mean_var()
        mean = mean.to(**self.tpdbv)
        var = var.to(**self.tpdbv)
        out = input_vector * \
            torch.sqrt(var)[(None,) * self.norm_axes] + \
